...
def main():
    ...
    preprocess, postprocess = make_pre_post_processors(policy.config, repo_id)
    ...
    try:
        while True:
            eef_pos = observation["robot_state"]["eef"]["pos"]
            eef_quat = observation["robot_state"]["eef"]["quat"]
            #print('quat',eef_quat,type(eef_quat))
            eef_quat_angle = _quat2axisangle(eef_quat.flatten())

            gripper = observation["robot_state"]["gripper"]["qpos"]
            # Concatenate into a single flattened NumPy array
            # Ensure they are concatenated in the order the policy expects (usually pos + quat + gripper)
            state_np = np.concatenate([eef_pos.flatten(), eef_quat_angle.flatten(), gripper.flatten()], axis=-1)
            state_vector = torch.from_numpy(state_np).to(device).float()
            #print(eef_pos,eef_quat,gripper)
            #print('state=',state_vector)
            #print('image=',observation["pixels"]["image"].shape)
            #print('image=',observation["pixels"]["image2"].shape)
            im = observation["pixels"]["image"]
            imt = im.transpose(0,3,1,2)
            im1 = observation["pixels"]["image2"]
            imt1 = im1.transpose(0,3,1,2)

            action_obs = {
                "observation.images.image": torch.from_numpy(imt).to(device).float(),
                "observation.images.image2": torch.from_numpy(imt1).to(device).float(),
                "observation.state":state_vector.unsqueeze(0), 
                "task": task_instruction,
            }
            #print('input feat=',policy.config.input_features)

            obs = preprocess(action_obs)

            #processed_obs = preprocess(observation)
            #print('post proc shape image =',obs["observation.images.image"].shape)
            #print('post proc shape image2 =',obs["observation.images.image2"].shape)

            # 2. Model Inference
            with torch.no_grad():
                action_chunk = policy.select_action(obs)
                real_action = postprocess(action_chunk)
            action_step = real_action[0, 0].cpu().numpy()     

            observation, reward, terminated, truncated, info = env.step(real_action.detach().cpu().numpy())
            cnt += 1
